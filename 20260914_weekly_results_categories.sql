...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.pa_ads'::regclass
      and conname = 'pa_ads_category_check'
  ) then
    alter table public.pa_ads
      add constraint pa_ads_category_check
      check (
        category in (
          'cosmetics_beauty',
          'fashion_apparel',
...
alter table public.pa_featured_ads_public
  add column if not exists category text not null default 'other';

alter table public.pa_featured_ads_public
  add column if not exists service_budget_usd numeric;
...
begin
  if new.is_featured is true then
    if new.featured_month is null or new.featured_year is null then
      new.featured_month := extract(month from timezone('Asia/Baghdad', now()))::smallint;
      new.featured_year := extract(year from timezone('Asia/Baghdad', now()))::smallint;
    end if;
    if new.featured_week_start is null then
      new.featured_week_start :=
...
update public.pa_ads
set featured_week_start =
  date_trunc('week', timezone('Asia/Baghdad', now()))::date
where is_featured is true
  and featured_week_start is null;
...
declare
  ...
  v_category text := lower(btrim(coalesce(p_ad->>'category', '')));
...
begin
  ...
  if coalesce(new.is_featured, false) then
    insert into public.pa_featured_ads_public (
      id, title, goal, category, video_link, thumbnail_url,
      clicks, impressions, spend, budget, total_budget, daily_budget, days,
      service_budget_usd, status, ad_number, created_at, is_featured,
      featured_month, featured_year, featured_week_start, featured_sort,
      updated_at
    ) values (
      new.id, new.title, new.goal, new.category, new.video_link,
      new.thumbnail_url, coalesce(new.clicks, 0),
      coalesce(new.impressions, 0), coalesce(new.spend, 0),
      new.budget, new.total_budget, new.daily_budget, new.days,
      new.service_budget_usd, new.status, new.ad_number, new.created_at,
      true, new.featured_month, new.featured_year,
      new.featured_week_start, new.featured_sort, now()
    )
    on conflict (id) do update set
      ...
      days = excluded.days,
      service_budget_usd = excluded.service_budget_usd,
      ...
      featured_week_start = excluded.featured_week_start,
...
revoke all on function public.pa_sync_featured_ad_public()
  from public, anon, authenticated;
grant execute on function public.pa_sync_featured_ad_public()
  to service_role;
...
select
  a.id, a.title, a.goal, a.category, a.video_link, a.thumbnail_url,
  coalesce(a.clicks, 0), coalesce(a.impressions, 0), coalesce(a.spend, 0),
  a.budget, a.total_budget, a.daily_budget, a.days,
  a.service_budget_usd, a.status, a.ad_number, a.created_at, true,
  a.featured_month, a.featured_year, a.featured_week_start, a.featured_sort,
  now()
from public.pa_ads a
where a.is_featured is true
on conflict (id) do update set
  title = excluded.title,
  goal = excluded.goal,
  category = excluded.category,
...
create index if not exists pa_ads_featured_week_order_idx
  on public.pa_ads (featured_week_start, featured_sort, impressions desc)
  where is_featured is true;

create index if not exists pa_featured_ads_public_week_order_idx
  on public.pa_featured_ads_public (
    featured_week_start,
    featured_sort,
    impressions desc
  )
...
comment on column public.pa_ads.category is
  'Stable category slug selected by the advertiser.';
comment on column public.pa_ads.featured_week_start is
  'Monday in Asia/Baghdad for admin-curated weekly results.';
comment on column public.pa_featured_ads_public.service_budget_usd is
  'Original daily budget multiplied by days, before discounts.';
